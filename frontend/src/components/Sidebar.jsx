...
const Sidebar = ({ user, setUser, vistaActual, setVista, handleLogout }) => {
  ...
  const cambiarStatus = async (e) => {
    ...
    const statusAnterior = user.status_tecnico; // Ahora usamos el nombre correcto

    setCambiandoStatus(true);
    // Cambiamos el estado de React instantáneamente
    setUser({ ...user, status_tecnico: nuevoStatus });

    try {
      // Mandamos la petición PUT a FastAPI
      await clienteAxios.put(`/usuarios/${user.id}`, {
        ...user,
        status_tecnico: nuevoStatus // Enviamos la variable correcta
      });
    } catch (error) {
      ...
      {['Admin', 'Tecnico'].includes(user?.rol) && (
        <div className="px-6 py-4 bg-slate-900 border-b border-slate-800">
          ...
          <div className="relative">
            <select
              value={user.status_tecnico || "Activo"} // Leemos la variable correcta
